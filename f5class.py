...
##            info.append(("-").join(self.get_time_stamp().split()))
    ...
    def test(self):
        ...
        try:
            ...
            errors += 1
            print "Test %s: Time Error Failed" % (test)

        
        test += 1
        if self.has_read("template"):
            try:
                self.get_mean_qscore("template")
            except:
                errors += 1
                print "Test %s: get mean qscore (template) failed." % (test)
            try:
                self.get_mean_qscore(self.use_molecule())
            except:
                errors += 1
                print "Test %s: get mean qscore (use_molecule) failed." % (test)
        if self._has_read["complement"]:
            try:
                self.get_mean_qscore("complement")
            except:
                errors += 1
                print "Test %s: get mean qscore (complement) failed." % (test)
            if self._has_read["2d"]:
                try:
                    self.get_mean_qscore("2d")
                except:
                    errors += 1
                    print "Test %s: Get Mean qscore (2d) failed" % (test)

        test += 1
        if self.has_read("template"):
            try:
                self.get_seq_len("template")
            except:
                errors += 1
                print "Test %s: get seq len (template) failed." % (test)
            try:
                self.get_seq_len(self.use_molecule())
            except:
                errors += 1
                print "Test %s: get seq len (use_molecule) failed." % (test)           
        if self._has_read["complement"]:
            try:
                self.get_seq_len("complement")
            except:
                errors += 1
                print "Test %s: get seq len (template) failed." % (test)
            if self._has_read["2d"]:
                try:
                    ...
                    errors += 1
                    print "Test %s: get seq len (2d) failed" % (test)


        test += 1
        try:
            self.get_num_events("input")
        except:
            errors += 1
            print "Test %s: get num events (input) failed." % (test)
        if self.has_read("template"):
            try:
                self.get_num_events("template")
            except:
                errors += 1
                print "Test %s: get num events (template) failed." % (test)
        if self._has_read["complement"]:
            try:
                self.get_num_events("complement")
            except:
                errors += 1
                print "Test %s: get num events (complement) failed." % (test)

        
        test += 1
        if self.has_read("template"):
            try:
                self.get_num_called_events("template")
            except:
                errors += 1
                print "Test %sa: get num called events (template) failed." % (test)
        if self._has_read["complement"]:
            try:
                self.get_num_called_events("complement")
            except:
                errors += 1
                print "Test %sb: get num called events (complement) failed." % (test)
        ...
        if self.has_read("template"):
            try:
                self.get_num_skips("template")
            except:
                errors += 1
                print "Test %sa: get num skips (template) failed." % (test)
        if self._has_read["complement"]:
            try:
                self.get_num_skips("complement")
            except:
                errors += 1
                print "Test %sb: get num skips (complement) failed." % (test)


        test += 1
        if self.has_read("template"):
            try:
                self.get_num_stays("template")
            except:
                errors += 1
                print "Test %sa: get num stays (template) failed." % (test)
        if self._has_read["complement"]:
            try:
                self.get_num_stays("complement")
            except:
                errors += 1
                print "Test %sb: get num stays (complement) failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get channel number." % (test)


        test += 1
        try:
            self.get_heatsink_temp()
        except:
            errors += 1
            print "Test 010: get heatsink temp failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get read id failed." % (test)
        ...
        if self.has_read("template"):
            try:
                self.get_fastq("template")
            except:
                errors += 1
                print "Test %s: get fastq (template) failed." % (test)
        if self.has_read("complement"):
            try:
                self.get_fastq("complement")
            except:
                errors += 1
                print "Test %s: get fastq (complement) failed." % (test)
        if self.has_read("2d"):
            try:
                self.get_fastq("2d")
            except:
                errors += 1
                print "Test %s: get fastq (2d) failed." % (test)


        test += 1
        if self.has_read("template"):
            try:
                self.get_fasta("template")
            except:
                errors += 1
                print "Test %s: get fasta (template) failed." % (test)
        if self.has_read("complement"):
            try:
                self.get_fasta("complement")
            except:
                errors += 1
                print "Test %s: get fasta (complement) failed." % (test)
        if self.has_read("2d"):
            try:
                self.get_fasta("2d")
            except:
                errors += 1
                print "Test %s: get fasta (2d) failed." % (test)


        test += 1
        if self.has_read("template"):
            try:
                self.get_quals("template")
            except:
                errors += 1
                print "Test %s: get quals (template) failed." % (test)
        if self.has_read("complement"):
            try:
                self.get_quals("complement")
            except:
                errors += 1
                print "Test %s: get quals (complement) failed." % (test)
        if self.has_read("2d"):
            try:
                self.get_quals("2d")
            except:
                errors += 1
                print "Test %s: get quals (2d) failed." % (test)


        test += 1
        if self.has_read("template"):
            try:
                self.get_quals_as_int("template")
            except:
                errors += 1
                print "Test %s: get quals as int (template) failed." % (test)
        if self.has_read("complement"):
            try:
                self.get_quals_as_int("complement")
            except:
                errors += 1
                print "Test %s: get quals as int (complement) failed." % (test)
        if self.has_read("2d"):
            try:
                self.get_quals_as_int("2d")
            except:
                errors += 1
                print "Test %s: get quals as int (2d) failed." % (test)

        test += 1
        try:
            self.get_tracking_id_version_name()
        except:
            errors += 1
            print "Test %s: get minknow version failed." % (test)
            

        test += 1
        try:
            self.get_event_detection_version()
        except:
            errors += 1
            print "Test %s: get event detection version failed." % (test)


        test += 1
        try:
            self.get_tracking_id_version()
        except:
            errors += 1
            print "Test %s: get tracking id version failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get protocol version failed." % (test)


        test += 1
        try:
            self.get_basecall_version()
        except:
            errors += 1
            print "Test %s: get basecall version failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get file version failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get device id failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get asic id failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get asic temp failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get run id failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get time stamp failed." % (test)


        test += 1
        try:
            self.get_start_time("experiment")
        except:
            errors += 1
        ...
        try:
            ...
            errors += 1
            print "Test %s: get start time (input) failed." % (test)
        try:
            self.get_start_time("template")
        except:
            errors += 1
            print "Test %s: get start time (template) failed." % (test)
        try:
            self.get_start_time("complement")
        except:
            errors += 1
            print "Test %s: get start time (complement) failed." % (test)
        ...
        try:
            ...
            errors += 1
            print "Test %s: get log failed." % test
        ...
        try:
            ...
            errors += 1
            print "Test %s: get log string failed." % test

        test += 1
        if self.has_read("template"):
            try:
                self.get_model("template")
            except:
                errors += 1
                print "Test %s: get model (template) failed." % test
        if self.has_read("complement"):
            try:
                self.get_model("complement")
            except:
                errors += 1
                print "Test %s: get model (complement) failed." % test

        test += 1
        if self.has_read("template"):
            try:
                self.get_model_string("template")
            except:
                errors += 1
                print "Test %s: get model string (template) failed." % test
        if self.has_read("complement"):
            try:
                self.get_model_string("complement")
            except:
                errors += 1
        ...
        if self.has_read("template"):
            try:
                ...
                errors += 1
                print "Test %s: get events (template) failed." % test
        if self.has_read("complement"):
            try:
                self.get_events("complement")
            except:
                errors += 1
        ...
        if self.has_read("template"):
            try:
                ...
                errors += 1
                print "Test %s: get events string (template) failed." % test
        if self.has_read("complement"):
            try:
                self.get_events_string("complement")
            except:
                errors += 1
        ...
        test += 1
        try:
            self.get_2d_alignment()
        except:
            errors += 1
            print "Test %s: get 2d alignment failed." % test

        test += 1
        try:
            self.get_2d_alignment_string()
        except:
            errors += 1
            print "Test %s: get 2d alignment string failed." % test

        test += 1
        try:
            self.get_strand_score("template")
        except:
            errors += 1
            print "Test %s: get strand score (template) failed." % test
        try:
            self.get_strand_score("complement")
        except:
            errors += 1
            print "Test %s: get strand score (complement) failed." % test

        test += 1
        try:
            self.get_num_stutters("template")
        except:
            errors += 1
            print "Test %s: get num stutters (template) failed." % test
        try:
            self.get_num_stutters("complement")
        except:
            errors += 1
            print "Test %s: get num stutters (complement) failed." % test

        test += 1
        try:
            self.get_min_tc_ratio()
        except:
            errors += 1
            print "Test %s: get min tc ratio failed." % test
        try:
            self.get_max_tc_ratio()
        except:
            errors += 1
            print "Test %s: get max tc ratio failed." % test
        try:
            self.get_tc_ratio_limits() 
        except:
            errors += 1
