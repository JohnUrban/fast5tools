...
DESCRIPTION
    ...
    Input = SAM file sorted by position using "samtools sort".
    -- FOR NOW, MUST BE SAM -- NOT BAM -- but can be STDIN SAM
    -- Treats all SAM records independently
    
    Output = bedGraph for all reads with >0 mismatches.
...
header = ["chr", "start", "end", "num_mismatches"]
print '\t'.join(header)
total = 0
totalAligned = 0
totalPerfect = 0
