    ...
    for f5 in Fast5List(args.fast5, keep_tar_footprint_small=args.tarlite):
        if f5.is_not_corrupt() and f5.is_nonempty:
            ...
            if read:
                ...
                ans = choose_barcode(barcodes, sw, read[1], search_start, search_end, ref_name=read[0], use_entire_barcode=False)
                aln1 = [str(ans[i]) for i in get]
                # If opted, look for second barcode on top strand
                if two_pos_barcodes:
                    ans2 = choose_barcode(barcodes2, sw, read[1], search_start2, search_end2, ref_name=read[0], use_entire_barcode=False)               
                ...
                if args.bothstrands:
                    ...
                    revans = choose_barcode(barcodes, sw, revcompseq, search_start, search_end, ref_name='revcomp_'+read[0], use_entire_barcode=False)
                    revaln1 = [str(revans[i]) for i in get]
                    # If opted, look for second barcode on top strand
                    if two_pos_barcodes:
                        revans2 = choose_barcode(barcodes2, sw, revcompseq, search_start2, search_end2, ref_name='revcomp_'+read[0], use_entire_barcode=False)               
