...
def get_formatted_pairwise_alignments(barcodechoice, getall=False):
    ...
    if getall:
        for barcode, alignment in barcodechoice[1].iteritems():
            outstring += get_formatted_pairwise_alignment(alignment)
        return outstring
    else:
        best = barcodechoice[1][barcodechoice[0][0]]
        return get_formatted_pairwise_alignment(best)
...
##        print
    outstring = ''
    outstring += "Ref (top): " + alignment.r_name + '\n'
    outstring += "Query (bottom): " + alignment.q_name + '\n'
    stats = ["Match:"+str(n_m), "Mismatch:"+str(n_mm), "Deletion:"+str(n_d), "Insertion:"+str(n_i), "AlignmentLength:"+str(len(ref)), "PercentIdentity:"+str(100.0*n_m/sum([n_m, n_mm, n_d, n_i]))]
