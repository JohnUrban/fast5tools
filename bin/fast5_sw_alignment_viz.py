    ...
    for f5 in Fast5List(args.fast5, keep_tar_footprint_small=args.tarlite):
        if f5.is_not_corrupt() and f5.is_nonempty:
            ...
            if read:
                ...
                ans = choose_barcode(barcodes, sw, read[1], search_start, search_end, ref_name=read[0], use_entire_barcode=False, return_alns=True)
                aln1 = [str(ans[0][i]) for i in get]
                out = ('\t').join( [">"+readname[0]] + aln1 + comments + seq + quals) + '\n'
                out += get_formatted_pairwise_alignments(ans, args.all)
                OUT.write( out + '\n' )
##                quit()
